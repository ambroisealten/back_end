...
public class SkillsBusinessController {
	...
	public HttpException createSkill(JsonNode jSkill, UserRole role) {
		return (UserRole.CDR_ADMIN == role || UserRole.MANAGER_ADMIN == role) ? skillEntityController.createSkill(jSkill) : new ForbiddenException();
	}
	...
	public HttpException updateSkill(JsonNode jSkill, UserRole role) {
		return (UserRole.CDR_ADMIN == role || UserRole.MANAGER_ADMIN == role) ? skillEntityController.updateSkill(jSkill) : new ForbiddenException();
	}
	 ...
	 *         and {@link CreatedException} if the skill is deleted
	 * @author Thomas Decamp
	 */
	public HttpException deleteSkill(JsonNode params, UserRole role) {
		return (UserRole.CDR_ADMIN == role || UserRole.MANAGER_ADMIN == role) ? skillEntityController.deleteSkill(params.get("name").textValue()) : new ForbiddenException();
	}
